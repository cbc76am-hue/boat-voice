...
# Synthetic boat position — HOME C08 Shelter Bay Marina slip.  Matches the
# operator's real GPS when the boat's at the dock.  Tolly's prompt rule
# (see prompts.py "Swinomish Channel exits") explicitly picks the north or
# south channel-exit mark as the PlanRoute start_lat/start_lon based on
# the destination + tide, so this sim coord doesn't need to be the
# router-routable point — it just needs to be where the boat actually is.
